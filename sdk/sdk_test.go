...
func TestConfigFilterByName(t *testing.T) {
	c := fakeConfigs.FilterByName("project")
	assert.Equal(t, 2, len(c.Contracts))
	c = fakeConfigs.FilterByName("yielder")
	assert.Equal(t, 3, len(c.Contracts))
	c = fakeConfigs.FilterByName("banegas_farm")
...
func TestConfigCall(t *testing.T) {
	...
	c := fakeConfigs.FilterByName("project").Contracts[0]
